(function (global, doc, ibexa, React, ReactDOM) {
    ...
    const onConfirm = (btn, items) => {
        ...
        const enhancedLinkSettingsWrapper = btn.closest('.ngenhancedlink-settings');

        toggleResetStartingLocationBtn(btn.parentNode.querySelector(SELECTOR_RESET_STARTING_LOCATION_BTN), true);

        enhancedLinkSettingsWrapper.querySelector(btn.dataset.relationRootInputSelector).value = locationId;
        enhancedLinkSettingsWrapper.querySelector(btn.dataset.relationSelectedRootNameSelector).innerHTML = locationName;
    };
